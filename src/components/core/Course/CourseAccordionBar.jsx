...
import CourseSubSectionAccordionBar from './CourseSubSectionAccordionBar'
    ...
    <div className=' overflow-hidden border border-solid border-richblack-600 bg-richblack-700 text-richblack-5 last:mb-0'>
      <div>
        ...
        onClick={() => {
          ...
          <div className=' space-x-4'>
            <span className=' text-yellow-25'>{`${course.subSection.length || 0} lectures`}</span>
          </div>
        </div>
      </div>
      ...
      >
        ...
        {course?.subSection?.map((subSec, i) => {
          return <CourseSubSectionAccordionBar subSec={subSec} key={i} />
        })}
        </div>
      </div>
    </div>
